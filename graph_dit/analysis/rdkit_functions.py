...
stats = {'connection_failure': 0, 'valency_check_failure_after_connection': 0}
...
class BasicMolecularMetrics(object):
    def __init__(self, atom_decoder, train_smiles=None, stat_ref=None, task_evaluator=None, n_jobs=8, device='cpu', batch_size=512):
        ...
        self.largest_molecule_failure = 0

    def compute_relaxed_validity(self, generated, ensure_connected):
        validity_checker = ValidityChecker(None, self.atom_decoder)
        valid = []
        num_components = []
        all_smiles = []
        valid_mols = []
        ...
        for graph in generated:
            ...
            mol = validity_checker.build_molecule_with_partial_charges(atom_types, edge_types, self.atom_decoder)
    
            direct_valid_flag = True if validity_checker.check_mol(mol, largest_connected_comp=True) is not None else False
            if direct_valid_flag:
                direct_valid_count += 1
            else:
                self.largest_molecule_failure += 1
    ...
    def evaluate(self, generated, targets, ensure_connected, active_atoms=None):
        ...
        print(f"Failure types/ largest molecule: {self.largest_molecule_failure} valency failure after connection: {stats['valency_check_failure_after_connection']} connection failure: {stats['connection_failure']}")
    ...
    while True:
        ...
        if flag:
            ...
            try:
                ...
                stats['valency_check_failure_after_connection']+=1
                return None, no_correct
