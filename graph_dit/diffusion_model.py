...
from models.gnn import Gnn_classifier
...
class Graph_DiT(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools):
        ...
        self.validity_layer = Gnn_classifier(hidden_size=self.Xdim, edge_dim=self.Edim)
    ...
    def forward(self, noisy_data, unconditioned=False):
        x, e, y = noisy_data['X_t'].float(), noisy_data['E_t'].float(), noisy_data['y_t'].float()
    ...
    def training_step(self, data, i):
        ...
        with torch.no_grad():
            pred_for_validity = self.forward(noisy_data, unconditioned=True)
            pred_for_validity = utils.PlaceHolder(
                X=pred.X.detach(),
                E=pred.E.detach(),
                y=pred.y.detach() if pred.y is not None else None
            ).mask(node_mask)

        if self.current_epoch % self.cfg.train.validity_drift_interval == 0 and i == self.drift_batch_idx:
            pred_collapsed = utils.PlaceHolder(X=pred_for_validity.X, E=pred_for_validity.E, y=pred_for_validity.y).mask(node_mask, collapse=True)
            validity_ground_label = utils.ValidityChecker(pred_collapsed, self.dataset_info.atom_decoder).generate_validity_scores()
            val_score = self.validity_layer(pred_for_validity.X, pred_for_validity.E)
            validity_ground_label = validity_ground_label.unsqueeze(-1).float().to(val_score.device)

            loss_fn = torch.nn.BCELoss()
            #logger.info(f"detached states X for pred validity {pred_for_validity.X.requires_grad} and E {pred_for_validity.E.requires_grad}")
            #logger.info(f"detached states for pred collapsed X {pred_collapsed.X.requires_grad} and E {pred_collapsed.E.requires_grad}")
            loss = loss_fn(val_score, validity_ground_label) # validation modules 0 means invalid

            opt2.zero_grad()
            assert all(p.grad is None for p in self.validity_layer.parameters())
            self.manual_backward(loss)
            opt2.step()
            
            self.log("classifier_validity_loss", loss, batch_size=X.size(0), sync_dist=True, on_step=True, on_epoch=True, prog_bar=True)

        loss_base = self.train_loss(masked_pred_X=pred.X, masked_pred_E=pred.E, pred_y=pred.y,
        ...
        self.manual_backward(loss_base)
        clip_grad_norm_(self.model.parameters(), max_norm=1)
        opt1.step()
        
        self.log(f'loss', loss_base, batch_size=X.size(0), sync_dist=True)
        return {'loss': loss_base}

    def configure_optimizers(self):
        validity_head_params = self.validity_layer.parameters()
        base_params = self.model.parameters()
        
        opt_base = torch.optim.AdamW(base_params, lr=self.cfg.train.lr, weight_decay=0.01)
        opt_validity_head = torch.optim.Adam(validity_head_params, lr=self.cfg.train.validation_lr) #TODO: Add val lr to config file
        
        return [opt_base, opt_validity_head]
    
    def on_fit_start(self) -> None:
        logger.info(f"Model device: {next(self.model.parameters()).device}")
        logger.info(f"validity_layer device: {next(self.validity_layer.parameters()).device}")

        logger.info(f'on fit train iteration: {self.train_iterations}')
        logger.info(f"Size of the input features Xdim {self.Xdim}, Edim {self.Edim}, ydim {self.ydim}")

    def on_train_epoch_start(self) -> None:
        self.drift_batch_idx = torch.randint(0, self.trainer.num_training_batches, (1,)).item()
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask):
        ...
        def get_prob(noisy_data, unconditioned=False, guidance=False):
            # print(f"[get_prob] unconditioned={unconditioned}, guidance={guidance}")
            with torch.inference_mode(False):
                with torch.enable_grad():
                    noisy_data['X_t'] = noisy_data['X_t'].detach().clone().float().requires_grad_(True)
                    noisy_data['E_t'] = noisy_data['E_t'].detach().clone().float().requires_grad_(True)
                    noisy_data['y_t'] = noisy_data['y_t'].detach().clone()
                    noisy_data['t'] = noisy_data['t'].detach().clone()
                    noisy_data['node_mask'] = noisy_data['node_mask'].detach().clone()
                    pred = self.forward(noisy_data, unconditioned=unconditioned)
            
                pred_X, pred_E = pred.X, pred.E

                sigma = ((1 - alpha_s_bar) / (1 - alpha_t_bar) * beta_t).unsqueeze(-1)

                if unconditioned and guidance:
                    assert pred_X.requires_grad == True
                    assert pred_E.requires_grad == True
                    pred_X, pred_E = diffusion_utils.targeted_guidance_computation_from_z0(guidance_classifier=self.validity_layer, 
                                                                                       pred_X=pred_X,
                                                                                       pred_E=pred_E,
        ...
        print(f"[sample_p_zs_given_zt] guidance_target={self.guidance_target}, guide_scale={self.guide_scale}")
        prob_X, prob_E = get_prob(noisy_data)

        ### Guidance
        if self.guidance_target is not None and self.guide_scale is not None:
            uncon_prob_X, uncon_prob_E = get_prob(noisy_data, unconditioned=True)
            uncon_guided_prob_X,  uncon_guided_prob_E = get_prob(noisy_data, unconditioned=True, guidance=True)
            prob_X = uncon_guided_prob_X *  (prob_X / uncon_prob_X.clamp_min(1e-10)) ** self.guide_scale  
            prob_E = uncon_guided_prob_E * (prob_E / uncon_prob_E.clamp_min(1e-10)) ** self.guide_scale  
