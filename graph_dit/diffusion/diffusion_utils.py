...
def targeted_guidance_computation_from_z0(guidance_classifier, pred_X, pred_E, guidance_scale, sigma):
    logits_X = pred_X.detach().requires_grad_(True)
    logits_E = pred_E.detach().requires_grad_(True)
    with torch.enable_grad():
        target_validity_score = guidance_classifier(logits_X, logits_E) # dim (B, 1)
        if target_validity_score.dim() > 0:
            target_validity_score = target_validity_score.mean()

    grad_X = torch.autograd.grad(target_validity_score, logits_X, retain_graph=True)[0]
    grad_E = torch.autograd.grad(target_validity_score, logits_E, retain_graph=True)[0]

    grad_X = grad_X / (grad_X.norm(dim=-1, keepdim=True)+1e-8)
    grad_E = grad_E / (grad_E.norm(dim=-1, keepdim=True)+1e-8)

    logits_flat = logits_X.flatten()
    grad_flat = grad_X.flatten()

    logits_dir = logits_flat / (logits_flat.norm() + 1e-8)

    grad_proj = torch.dot(grad_flat, logits_dir).abs() * logits_dir

    grad_proj = grad_proj.view_as(grad_X)
    
    pred_X = logits_X + guidance_scale*sigma*grad_proj

    logits_flat = logits_E.flatten()
    grad_flat = grad_E.flatten()

    logits_dir = logits_flat / (logits_flat.norm() + 1e-8)

    grad_proj = torch.dot(grad_flat, logits_dir).abs() * logits_dir

    grad_proj = grad_proj.view_as(grad_E)

    sigma = sigma.unsqueeze(-1)
    pred_E = logits_E + guidance_scale*sigma*grad_proj
